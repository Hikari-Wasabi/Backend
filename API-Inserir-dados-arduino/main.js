...
const serial = async (
    ...
    let portaArduino = portas.find((porta) => porta.vendorId == 2341 && porta.productId == 43);
    if (!portaArduino) {
        // verifica outras portas caso o arduino não seja original
        portaArduino = portas.find((porta) =>
        porta.path.includes("ttyUSB") || porta.path.includes("ttyACM")
        );
        if (!portaArduino) {
    ...
    arduino.pipe(new serialport.ReadlineParser({ delimiter: '\r\n' })).on('data', async (data) => {
        ...
        if (HABILITAR_OPERACAO_INSERIR) {
            ...
            await poolBancoDados.execute(
                'INSERT INTO wasabi_daily (valor_umidade, valor_temperatura, fk_Sensor) VALUES (?, ?, ?)',
                [sensorUmidade, sensorTemperatura, '100']
            );
